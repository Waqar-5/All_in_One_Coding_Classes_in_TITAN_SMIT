...
/**
 * Drag & drop / click-to-browse image uploader. Used for book covers and,
 * with the props below, profile avatars.
 *
 * Props:
 ...
 * - label: field label text (default "Book cover")
 * - previewAlt: alt text for the preview image
 * - shape: "rect" (default, book cover) or "circle" (profile avatar)
 * - heightClass: Tailwind height class for the preview/dropzone (default "h-56")
 */
export default function ImageUpload({
  ...
  const shapeClass = shape === "circle" ? "rounded-full aspect-square" : "rounded-xl";
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-semibold text-ink-500 dark:text-paper-200">{label}</label>

      {previewUrl ? (
        <div className={`group relative overflow-hidden border border-ink-200 dark:border-paper-400/20 ${shapeClass} ${shape === "circle" ? "mx-auto w-32" : ""}`}>
          <img
            src={previewUrl}
            alt={previewAlt}
